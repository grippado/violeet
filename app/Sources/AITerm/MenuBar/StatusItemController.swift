...
// aiterm — which is precisely when you did not need answering. The status item
...
final class StatusItemController: NSObject, NSMenuDelegate {
    ...
    init(state: AppState) {
        ...
        statusItem.button?.setAccessibilityLabel("aiterm sessions")
    ...
    /// that can say so while aiterm is not in front.
    ...
    private func refreshIcon(force: Bool = false) {
        ...
        let description = waiting > 0
            ? "aiterm — \(waiting) waiting for you"
            : "aiterm"
    ...
    func menuNeedsUpdate(_ menu: NSMenu) {
        ...
        let open = NSMenuItem(title: "Open aiterm", action: #selector(openApp), keyEquivalent: "")
        open.target = self
        menu.addItem(open)
        ...
        let quit = NSMenuItem(title: "Quit aiterm", action: #selector(quit), keyEquivalent: "")
        quit.target = self
        menu.addItem(quit)
    }
    ...
    private func elsewhereItem(for card: SessionCard) -> NSMenuItem {
        ...
        item.toolTip = card.originApp.map {
            "Running in \($0). aiterm cannot focus it yet — it did not start this session."
        } ?? "aiterm did not start this session and cannot focus it."
        return item
    }
    ...
    private var mainWindow: NSWindow? {
        NSApp.windows.first { $0.frameAutosaveName == "aiterm.main" }
