// `~/.violeet/daemon.json` — the file clients read instead of guessing.
//
// The app only needs the socket path, and the protocol already fixes that at
// `~/.violeet/daemon.sock`. Reading the file anyway buys two things the constant
...
enum Discovery {
    ...
    /// `~/.violeet/daemon.sock`, the path the protocol fixes.
    static var defaultSocketPath: String {
        homeDirectory.appendingPathComponent(".violeet/daemon.sock").path
    }

    static var discoveryFilePath: String {
        homeDirectory.appendingPathComponent(".violeet/daemon.json").path
    }
