...
// file is the bug — the same rule `crates/violeet-proto` states for the Rust
...
struct SessionRegistered: Equatable {
    ...
    /// outside violeet. A real, supported state, not an error (ADR-003).
    let tabID: String?
    let agent: String
    let cwd: String?
    let title: String?
...
struct SessionUpdated: Equatable {
    ...
    /// Where a session violeet did not start is actually running: the terminal
