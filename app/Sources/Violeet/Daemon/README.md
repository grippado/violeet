...
Connects to `~/.violeet/daemon.sock`, reads JSON-lines, decodes by the `type`
...
it; when the two disagree, the document wins. `crates/violeet-proto` is the Rust
...
| `Discovery.swift` | Reads `~/.violeet/daemon.json`. |
