...
// quantity, from a source violeet does not have. Unlabelled numbers do not read
...
struct SessionCardView: View {
    ...
    private var compactSummary: String {
        ...
        if card.tabID == nil, let origin = card.originLabel {
            lines.append("Running in \(origin), outside violeet.")
        }
        lines.append("")
    ...
    /// Where this session lives, for one violeet did not start.
    ///
    ...
    private func originRow(_ origin: String) -> some View {
        Label(origin, systemImage: "terminal")
            ...
            .help("This session is running in \(origin), outside violeet.")
    }

    private var titleRow: some View {
        HStack(spacing: 5) {
            // A session with no tab is one violeet did not launch — an agent
            ...
            if card.tabID == nil, card.originLabel == nil {
                Image(systemName: "arrow.up.forward.app")
                    ...
                    .help("Running outside violeet — there is no tab to switch to.")
            }
            EditableName(
                name: name,
                display: title,
    ...
    /// entirely, from a source violeet does not even have. A bar with a
    ...
    private var contextHelp: String {
        guard card.contextFraction != nil else {
            ...
            The window's total size is not something violeet can see for this \
        ...
        This is not your plan's usage limit; violeet does not have that number.
        """
    }
    ...
    private func tokenHelp(partial: Bool) -> String {
        let scope = partial
            ? "Counted from when violeet started watching this session, not from its start — the real totals are higher."
