...
// `PATH`. That copy is also how state belonging to whatever launched aiterm
// reaches every session aiterm starts, which is a bug with teeth: measured on a
// running build, an aiterm opened from inside a Claude Code session handed
...
struct ChildEnvironmentTests {
    ...
    @Test func the_tab_id_is_this_tab_and_the_socket_is_ours() {
        ...
        #expect(env["AITERM_TAB_ID"] == "tab-abc")
        #expect(env["AITERM_SOCKET"] == "/tmp/daemon.sock")
    }
    ...
    @Test func agent_session_state_does_not_reach_the_child() {
        ...
        for name in TerminalSession.inheritedAgentState {
            // The two aiterm sets itself are in the list so an aiterm launched
            // from an aiterm tab cannot pass on the old tab's id; they are then
            // written back with this tab's values.
            if name.hasPrefix("AITERM_") { continue }
    ...
    @Test func the_things_a_terminal_needs_are_still_there() {
        ...
        #expect(env["TERM_PROGRAM"] == "aiterm")
