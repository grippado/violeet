//! What an aiterm hook entry looks like, and how to recognize one.
//!
//! # Two URLs, not eleven
//!
...
//!
//! Confirmed by reading `crates/aiterm-daemon/src/http/mod.rs`, not by
...
//!
//! The tab binding (ADR-003) rides in the `x-aiterm-tab-id` header as
//! `$AITERM_TAB_ID`. Claude Code only interpolates an environment variable into
...
/// How an aiterm entry is recognized on the way back out.
///
...
/// `/hook/event?src=aiterm` and `/hook/event` reach the same handler, and the
/// marker survives a port change because it is matched independently of it.
pub const MARKER: &str = "src=aiterm";
...
/// The header carrying `AITERM_TAB_ID`, spelled as the daemon reads it.
const TAB_ID_HEADER: &str = "x-aiterm-tab-id";
const TAB_ID_ENV: &str = "AITERM_TAB_ID";
...
mod tests {
    ...
    fn every_entry_declares_the_env_var_it_interpolates() {
        for event in all_events() {
            ...
            assert_eq!(header, "$AITERM_TAB_ID");

            let allowed = hook["allowedEnvVars"].as_array().unwrap();
            assert!(
                allowed.iter().any(|v| v == TAB_ID_ENV),
                "{event}: a header referencing $AITERM_TAB_ID that is not in \
                 allowedEnvVars resolves to an empty string, and an empty \
                 x-aiterm-tab-id is read by the daemon as no tab at all"
            );
        }
    }
    ...
    fn informational_events_share_one_url_and_permission_has_its_own() {
        ...
        assert!(informational[0].ends_with("/hook/event?src=aiterm"));

        let permission = entry_for(PERMISSION_EVENT, 9847);
        assert_eq!(
            permission["hooks"][0]["url"],
            "http://127.0.0.1:9847/hook/permission-request?src=aiterm"
        );
    }
