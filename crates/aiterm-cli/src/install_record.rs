//! What aiterm has installed, so `doctor` can notice when it vanishes.
//!
...
//! scratch each time — which silently removes anything aiterm wrote. The user
...
fn path() -> Option<PathBuf> {
    crate::probe::aiterm_dir().map(|d| d.join("installed.json"))
}
