...
//! 3. only then write `~/.aiterm/daemon.json`
//!
...
use aiterm_daemon::discovery::{self, DaemonInfo};
use aiterm_daemon::http::{HookServer, DEFAULT_HOOK_PORT};
use aiterm_daemon::registry::Registry;
use aiterm_daemon::socket::{default_socket_path, Hub, SocketServer};
use aiterm_daemon::wire::EndReason;
...
const PORT_ENV: &str = "AITERM_HOOK_PORT";
...
async fn main() -> ExitCode {
    ...
    eprintln!("aiterm-daemon: listening on {}", socket.path().display());

    let hooks = match HookServer::bind(hook_port(), hub.clone()).await {
        Ok(server) => server,
        Err(e) => {
            eprintln!("aiterm-daemon: cannot bind the hook endpoint: {e}");
            return ExitCode::FAILURE;
        }
    };
    let hook_port = hooks.port();
    eprintln!("aiterm-daemon: hooks on http://127.0.0.1:{hook_port}");
    ...
    if let Some(path) = &discovery_path {
        ...
        if let Err(e) = discovery::write(path, &info) {
            ...
            eprintln!("aiterm-daemon: could not write {}: {e}", path.display());
        }
    }
    ...
    tokio::spawn(async move {
        if let Err(e) = hooks.serve().await {
            eprintln!("aiterm-daemon: hook endpoint stopped: {e}");
        }
    });
    ...
    let outcome = tokio::select! {
        ...
        _ = shutdown_signal() => {
            eprintln!("aiterm-daemon: shutting down");
            ExitCode::SUCCESS
        }
    };
...
fn hook_port() -> u16 {
    match std::env::var(PORT_ENV) {
        Ok(raw) => raw.parse().unwrap_or_else(|_| {
            eprintln!("aiterm-daemon: {PORT_ENV}={raw:?} is not a port; using the default");
            DEFAULT_HOOK_PORT
        }),
...
/// leave `~/.aiterm/daemon.json` behind on every managed shutdown — and a stale
...
async fn shutdown_signal() {
    ...
    let mut term = match signal(SignalKind::terminate()) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("aiterm-daemon: cannot listen for SIGTERM: {e}");
...
async fn sweeper(hub: Hub) {
    ...
    loop {
        ...
        if expired_hitl > 0 {
            eprintln!("aiterm-daemon: {expired_hitl} permission request(s) timed out");
        }
