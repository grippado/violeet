...
use aiterm_daemon::hitl::HitlRegistry;
use aiterm_daemon::http::{HookServer, HARNESS_HEADER, TAB_ID_HEADER};
use aiterm_daemon::registry::{Registry, SessionState};
...
async fn a_hook_with_no_tab_id_registers_an_unbound_session_rather_than_failing() {
    ...
    // Header present but empty, which is what `curl -H "...: $AITERM_TAB_ID"`
