...
//! - `"Você é a TRACK B do projeto aiterm. Outras tracks rodam em paralelo…"` —
...
mod tests {
    ...
    fn real_first_prompts_become_readable_titles() {
        let cases: &[(&str, &str)] = &[
            ...
            (
                "Você é a TRACK B do projeto aiterm. Outras tracks rodam em paralelo.",
                "Você é a TRACK B do projeto aiterm",
            ),
        ];
    ...
    fn a_slash_command_names_itself() {
        assert_eq!(from_prompt("/flux:build aiterm ENG-1").as_deref(), Some("/flux:build"));
