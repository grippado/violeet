//! `violeet sessions` — what the daemon currently thinks is running.
//!
//! # Why this exists
//!
...
//!
//! The messages are parsed as plain JSON rather than through `violeet-proto`'s
...
fn fetch(path: &std::path::Path) -> Result<BTreeMap<String, Row>, String> {
    ...
    writeln!(
        ...
        violeet_proto::wire::timestamp(chrono_now())
    )
...
mod tests {
    ...
    fn a_session_with_no_title_falls_back_to_the_working_directory_and_says_so() {
        let row = Row {
            cwd: Some("/Users/x/www/personal/violeet".into()),
            ..Row::default()
        };
        assert_eq!(title_of(&row), "violeet (cwd)");
    }
