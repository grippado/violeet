        ...
        match self {
            ...
            Self::NotRoundTrippable { path, why } => write!(
                ...
                "{} cannot be edited safely: {why}\n\n\
                 violeet will not rewrite a file it would have to reformat. Claude Code \
...
impl Settings {
    ...
    pub fn back_up(&self) -> Result<Option<PathBuf>, SettingsError> {
        ...
        let backup = self
            .path
            .with_extension(format!("json.violeet-bak-{stamp}"));
    ...
    pub fn write(&self) -> Result<(), SettingsError> {
        ...
        let tmp = self.path.with_extension("json.violeet-tmp");
    ...
    /// Add our entry for `event`, replacing any previous violeet entry.
    ///
    ...
    ///
    /// Front, because a hook that was there before violeet arrived should not be
    ...
    if looks_like_jsonc(text) {
        format!(
            "it uses JSONC syntax (comments and/or trailing commas), which \
             Claude Code accepts but which violeet cannot rewrite without \
             discarding — {error}"
        )
    } else {
