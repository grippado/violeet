...
/// `~/.violeet/daemon.json`, or `None` if we cannot tell where home is.
pub fn default_path() -> Option<PathBuf> {
    std::env::var_os("HOME")
        .map(|home| PathBuf::from(home).join(violeet_proto::DAEMON_JSON_RELATIVE_PATH))
}

/// Write the file, creating `~/.violeet/` if needed.
///
