//! Where a session is actually running, when it is not in an violeet tab.
//!
//! # The problem
//!
//! violeet sees every Claude Code session on the machine, not only the ones it
...
//! about where it lived. "Running outside violeet" is true and useless.
//!
...
pub struct Origin {
    ...
    /// `Terminal`, `violeet`, `tmux: server`. Reported verbatim rather than
...
mod tests {
    ...
    #[test]
    fn a_session_started_by_violeet_says_so() {
        let t = table(&[
            (92112, 92111, Some("ttys010"), "claude"),
            (92111, 92110, Some("ttys010"), "-zsh"),
            (92110, 1, None, "violeet"),
            (1, 0, None, "launchd"),
        ]);
        assert_eq!(walk(92112, &t).app.as_deref(), Some("violeet"));
    }
