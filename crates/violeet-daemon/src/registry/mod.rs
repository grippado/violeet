...
//! hook can win the race, and an agent started outside violeet never sends one at
//! all.
...
pub struct HookObservation {
    ...
    /// From `VIOLEET_TAB_ID` in the hook payload's environment. `None` for an
    /// agent started outside violeet.
...
mod tests {
    ...
    fn unbound_session_is_matched_by_a_later_hook_once_the_tab_is_known() {
        ...
        // A hook with no tab id at all: an agent started outside violeet.
