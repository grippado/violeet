...
use violeet_daemon::registry::{Harness, HookObservation, Registry, SessionState, TitleSource};
use violeet_daemon::socket::{Hub, SocketServer};
use violeet_daemon::wire::{DaemonToApp, EndReason, SessionEnded, PROTOCOL_VERSION};
use chrono::Utc;
...
async fn start() -> Harnessed {
    ...
    // `~/.violeet/`. Not hypothetical: the rename test below wrote `"s1": "my
    ...
    unsafe {
        std::env::set_var("VIOLEET_STATE_DIR", dir.path());
    }
...
fn session_ended(session_id: &str) -> DaemonToApp {
    DaemonToApp::SessionEnded(SessionEnded {
        v: PROTOCOL_VERSION,
        ts: violeet_daemon::wire::timestamp(Utc::now()),
