...
//! reimplementing the shapes. That is the point of it living in `violeet-proto`:
...
pub struct SessionUpdated {
    ...
    /// a client can tell a name the human chose from one violeet derived. The
    ...
    // Where a session is actually running, for the ones violeet did not start.
    ...
    // session in an violeet tab, which needs no "where" — the tab is the where.
