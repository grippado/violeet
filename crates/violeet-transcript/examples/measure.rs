...
//!
//! `cargo run -p violeet-transcript --example measure -- <path>`

use violeet_transcript::{claude_code::ClaudeCodeReader, TranscriptSession};
...
fn main() {
    ...
    let head = violeet_transcript::claude_code::scan_head(std::path::Path::new(&path)).unwrap();
    println!("head.ai_title  = {:?}", head.ai_title);
    println!("head.derived   = {:?}", head.first_prompt.as_deref().and_then(violeet_transcript::title::from_prompt));
}
